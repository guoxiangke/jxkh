...
class UploadHandler
{
    ...
    /**
     * Add the file to the Filevault
     *
     *   // ffmpeg -i An\ Inconvenient\ Truth\ \(2006\).mp4 -ss 10 -vframes 1 shot_10.jpg
     *   // Also look into ffmpeg-php
     *   // Yes, add the file to the filevault
     *
     *
     */

    protected function add_to_filevault(&$content){

        if($content[0]->size % 1000000 == 0){
            return;
        }

        global $user;

        $tempFile = DRUPAL_ROOT.'/'.urldecode($content[0]->url);
        ...
        $file = file_move($file,'public://filevault');
        ...
        // Update info set to script
        if(!strpos( $entity->mime_type,"mp4" )){
            $content[0]->embedded = image_style_url("embedded", $file->uri);
        }
        $content[0]->original = file_create_url($file->uri);
        $content[0]->filename = $file->filename;
        $content[0]->entity = drupal_json_encode($entity);
    }
